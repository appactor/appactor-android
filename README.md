...
```kotlin
dependencies {
    implementation("com.appactor:appactor-android:2.3.4")
}
```
...
System profile context is collected automatically during `configure()`. The SDK
sends canonical `$...` profile keys such as app version, SDK version, locale,
timezone, platform, and device model into profile-current storage instead of the
custom attributes list. `collectDeviceIdentifiers()` remains opt-in for
